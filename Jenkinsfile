               ...
               dir('greencandle') {
                   ...
                   script {
                       ...
                       for (interval in arr) {
                           ...
                           export id=${BUILD_ID}
                           export test=strat-${BUILD_ID}
                           docker exec unit-runner-$BUILD_ID bash -c 'backend_test -i $interval -d /data/altcoin_historical/${year}/x -p $pair -s 2>&1 | tee /data/output/${name}/${pair}-${interval}-${year}.log'
